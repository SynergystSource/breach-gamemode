...
function SpawnAllItems()
	...
	local k2mul = 2
	if #player.GetAll() > 16 and #player.GetAll() < 26  then
		k2mul = 1.55
	elseif #player.GetAll() > 25 then
		k2mul = 1.2
	end
	local resps2 = table.Copy(SPAWN_KEYCARD2)
	local num = math.Round(#SPAWN_KEYCARD2 / k2mul)
	print("spawning " .. num .. " level 2 keycards")
	for i=1,num do
		local item = ents.Create( "keycard_level2" )
		if IsValid( item ) then
			local spawn = table.Random(resps2)
			item:Spawn()
			item:SetPos( spawn )
			table.RemoveByValue(resps2, spawn)
		end
	end
	...
	local resps3 = table.Copy(SPAWN_KEYCARD3)
	local num2 = math.Round(#SPAWN_KEYCARD3 / k3mul)
	print("spawning " .. num2 .. " level 3 keycards")
	for i=1,num2 do
		local item = ents.Create( "keycard_level3" )
		if IsValid( item ) then
			local spawn2 = table.Random(resps3)
			item:Spawn()
			item:SetPos( spawn2 )
			table.RemoveByValue(resps3, spawn2)
		end
	end
	
	
	local k4num = GetConVar("br_spawn_level4"):GetInt()
	k4num = math.Clamp(k4num, 1, #SPAWN_KEYCARD4)
	print("spawning "..k4num.." level 4 keycard")
	local resps4 = table.Copy(SPAWN_KEYCARD4)
	for i=1, k4num do
		local item = ents.Create( "keycard_level4" )
		if IsValid( item ) then
			local spawn3 = table.Random(resps4)
			item:Spawn()
			item:SetPos( spawn3 )
			table.RemoveByValue(resps4, spawn3)
		end
	end
	
	local resps5 = table.Copy(SPAWN_ITEMS)
	
	local item = ents.Create( "weapon_mtf_usp" )
	if IsValid( item ) then
		local spawn4 = table.Random(resps5)
		item:Spawn()
		item:SetPos( spawn4 )
		item.SavedAmmo = item.Primary.ClipSize
		table.RemoveByValue(resps5, spawn4)
	end
